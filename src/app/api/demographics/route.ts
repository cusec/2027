...
const manyWithOther = (
  ...
  const other = values.includes(OTHER)
    ? text(answers, `${field}Other`, required)
    : "";
...
function background(answers: Answers, attendeeType: string): Update {
  ...
  if (SCHOOL_TYPES.includes(attendeeType)) {
    ...
    if (school === OTHER) {
      ...
      if (campus && !institution.campuses?.includes(campus))
        throw new InvalidAnswer("campus");
  ...
  if (STUDIES_TYPES.includes(attendeeType)) {
    ...
    update.expectedGraduation = one(
      answers,
      "expectedGraduation",
      GRADUATION_OPTIONS,
      true,
    );
    update.internships = one(
      answers,
      "internships",
      INTERNSHIP_COUNT_OPTIONS,
      true,
  ...
  if (WORK_TYPES.includes(attendeeType)) {
    Object.assign(
      update,
      oneWithOther(answers, "currentRole", CURRENT_ROLE_OPTIONS, true),
    );
...
function goals(answers: Answers): Update {
  const opportunities = manyWithOther(
    answers,
    "opportunities",
    OPPORTUNITY_OPTIONS,
    undefined,
  ...
  return {
    ...
    workLocations: looking
      ? many(
          ...
          "workLocations",
          WORK_LOCATION_OPTIONS,
          LIMITS.workLocations,
          true,
        )
      : [],
    workArrangement: looking
      ? one(answers, "workArrangement", WORK_ARRANGEMENT_OPTIONS, true)
      : "",
  };
}
...
function travel(answers: Answers): Update {
  ...
  if (delegation === "yes") {
    ...
    if (delegationSchool === OTHER)
      delegationOther = text(answers, "delegationOther", true);
  }
...
function experience(answers: Answers): Update {
  ...
  if (attended.includes(FIRST_TIME) && attended.length > 1)
    throw new InvalidAnswer("attended");
...
export async function GET(request: Request) {
  if (isLocalTicketPreview(new URL(request.url).hostname)) {
...
export async function PUT(request: Request) {
  const localPreview = isLocalTicketPreview(new URL(request.url).hostname);
  const session = localPreview ? null : await auth0.getSession();
  const sessionEmail = session?.user?.email;
  if (!localPreview && !sessionEmail) {
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }
  ...
  if (
    section === "background" &&
    !localPreview &&
    !existing?.profile?.sections?.basics
  ) {
    return NextResponse.json(
      { error: "Save the basics first" },
      { status: 409 },
    );
  }
    ...
    switch (section) {
      ...
      case "background":
        update = background(
          answers,
          localPreview
            ? one(answers, "attendeeType", ATTENDEE_TYPE_OPTIONS, true)
      ...
      case "experience":
        ...
        break;
      case "links":
        update = links(answers, Boolean(existing?.profile?.sponsorConsent));
        break;
    }
  } catch (error) {
    if (error instanceof InvalidAnswer) {
      return NextResponse.json(
        { error: "invalid-answer", field: error.field },
        { status: 400 },
      );
    }
    throw error;
  ...
  const wasProfileComplete = Boolean(
    profileSections?.basics && profileSections?.background,
  );
  if (!wasProfileComplete && done("basics") && done("background")) {
    // The controlled option value - "other" for the Other option, never the
    // free-text attendeeTypeOther answer.
    const attendeeType =
      section === "basics"
  ...
  const wasRegistrationComplete = ALL_SECTIONS.every((id) =>
